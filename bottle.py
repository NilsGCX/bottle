...
import ast, base64, cgi, email.utils, functools, hmac, imp, itertools, mimetypes, os, re, tempfile, threading, time, warnings, weakref, hashlib
        ...
        if secret:
            ...
            if value and value.startswith("!") and "?" in value:
                ...
                if _lscmp(sig, base64.b64encode(hash)):
                    # GCX annotation
                    # As long as secret is non-corrupted, using pickle.loads is
                    # save in this context.
                    dst = pickle.loads(base64.b64decode(msg))  # nosec
...
class BaseResponse(object):
    ...
    def copy(self, cls=None):
        ...
        if not issubclass(cls, BaseResponse):
            raise TypeError("%s is no subclass of %s" % (cls, BaseResponse))
        copy = cls()
...
def static_file(
    ...
    if etag is None:
        ...
        # GCX annotation
        # SHA1 is marked as insecure hash function. Although this is true,
        # ETags cannot switch to the SHA2 family due to backwards compatibility
        # reasons. As of yet securing with SHA1 should be sufficient
        etag = hashlib.sha1(tob(etag)).hexdigest()  # nosec
...
def cookie_decode(data, key, digestmod=None):
    ...
    if cookie_is_encoded(data):
        ...
        if _lscmp(sig[1:], base64.b64encode(hashed)):
            # GCX annotation
            # See line 1332
            return pickle.loads(base64.b64decode(msg))  # nosec
    return None
...
class CGIServer(ServerAdapter):
    ...
    def run(self, handler):  # pragma: no cover
        # GCX annotation
        # The potential httpoxy vulnerability has been tested and cannot be
        # applied in this case, both with the dev-server and gunicorn.
        # (see https://httpoxy.org)
        from wsgiref.handlers import CGIHandler  # nosec

        def fixed_environ(environ, start_response):
            environ.setdefault("PATH_INFO", "")
            return handler(environ, start_response)

        CGIHandler().run(fixed_environ)  # nosec
...
def load(target, **namespace):
    ...
    return ast.literal_eval("%s.%s" % (module, target), namespace)
    ...
    if reloader and not os.environ.get("BOTTLE_CHILD"):
        ...
        try:
            ...
            while os.path.exists(lockfile):
                ...
                # GCX annotation
                # As long as os.environ is not corrupted during app start-up,
                # the subprocess.Popen call is save in this context.
                p = subprocess.Popen(args, env=environ)  # nosec
...
class MakoTemplate(BaseTemplate):
    """
    GCX annotation
    Mako templates do not automatically escape HTML/JS etc. and are therefore
    susceptible for Cross Site Scripting attacks. Therefore all variables must
    ...
    def prepare(self, **options):
        ...
        if self.source:
            self.tpl = Template(self.source, lookup=lookup, **options)  # nosec
        else:
...
class Jinja2Template(BaseTemplate):
    def prepare(self, filters=None, tests=None, globals={}, **kwargs):
        ...
        # GCX annotation
        # Set autoescape to True to avoid Cross Site Scripting attacks.
        self.env = Environment(
            autoescape=True, loader=FunctionLoader(self.loader), **kwargs
        )
        if filters:
...
class SimpleTemplate(BaseTemplate):
    """
    GCX annotation
    The SimpleTemplate uses exec and is therefore susceptible for
    harmful Python code and should not be used on untrusted templates.
    """
    ...
    def execute(self, _stdout, kwargs):
        ...
        # GCX annotation
        # See docstring for SimpleTemplate cls
        exec(self.co, env)  # nosec
        if env.get("_rebase"):
